...
// Size of arrays used
constexpr size_t arraySizes[nTests] = {512 * 1024,        // 2MB / array = 6MB (L3 fit)
                                       4   * 1024 *1024,  //  16MB / array =  48MB (RAM fit)
                                       16  * 1024 *1024}; //  64MB / array =  96MB (RAM fit)

// Number of benchmark repetitions
constexpr size_t testRept[nTests]   = {100000, 10000, 1000};
// Max array size
...
{
  // NUMA first touch
  #pragma omp parallel for simd schedule(simd:static) aligned(array:64)
...
void vectorAdd(float* c,
               ...
               float* b)
{
  #pragma omp parallel for simd schedule(simd:static) aligned(a,b,c:64)
  for (size_t i = 0 ; i < size; i++)
  {
    c[i] = a[i] + b[i];
  }
...
{
  float res = 0;
  #pragma omp parallel for simd schedule(simd:static) aligned(a,b,c:64) reduction(+:res)
  for (size_t i = 0 ; i < size; i++)
  {
    res += a[i] * b[i];
  }
