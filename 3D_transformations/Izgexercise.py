...
class Generate(bpy.types.Operator):
    ...
    def createView():
        # position of camera
        eyePosition = mathutils.Vector((0.0, 1.0, 0.0))
        # point where the camera looks at (center of the scene)
        centerPosition = mathutils.Vector((0.0, 0.0, 0.0))
        Z = centerPosition - eyePosition
        Z.normalize()
        Z = Z @ mathutils.Matrix.Rotation(math.radians(-13.0), 3, 'Z')
    ...
    # this function returns model matrix for specific object
    def createTransformation(object, view):
        ...
        objectsCount = len(bpy.data.collections[0].all_objects) - 1  # - 1 because camera is also object
        #                                           ^ yeah it could be constant but nobody likes magic constants

        rotation = mathutils.Matrix.Rotation(math.radians(90.0), 4, 'X')
        transformation = mathutils.Matrix.Translation((objectsCount - objectNumber, 0, 0))

        transformation = view @ transformation @ rotation
        transformation @= mathutils.Matrix.Translation((-(objectsCount / 2), 0, 0))  # centering
    ...
    def applyTransformation(transformation, object):
        for vertex in object.data.vertices:
            vertex.co = mathutils.Vector(vertex.co) @ transformation
...
def register():
    ...
    bpy.types.Scene.modelmatrix = bpy.props.BoolProperty(name="Apply model matrices",
                                                         description="Enables the model matrix multiplication part")
    bpy.types.Scene.viewmatrix = bpy.props.BoolProperty(name="Apply view matrix",
                                                        description="Enables the model view matrix multiplication part")
